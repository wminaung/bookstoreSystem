...
public class StaffLogin extends JFrame {
	...
	public StaffLogin() {
		...
		btnLogin.addKeyListener(new KeyAdapter() {
			...
			public void keyPressed(KeyEvent e) {
				if(e.getKeyCode() == KeyEvent.VK_ENTER) {
					login(txt1,txt2,pwd,isAdmin,StaffLogin.this);
					return;
				}
			}
		});
	...
	public static void login(JTextField txt1,JTextField txt2,JPasswordField pwd,boolean isAdmin, StaffLogin sl) {
		...
		try {
			...
			while (rs.next()) {
				...
				if (usrName.equals(name) && usrEmail.equals(email) && usrPass.equals(password)) {
					...
					// go view Book
					ViewBook view = new ViewBook(isAdmin, user_id);
			...
			if(!isValid) {
				JOptionPane.showMessageDialog(null, "You are something wroung", "error",
